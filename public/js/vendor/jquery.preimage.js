  ...
  var methods = {
     init : function( options ) {
		...
		return this.each(function(){
			...
			$('#'+settings['prefix']+this.id).html('').addClass(settings['prefix']+'container' + this.id);
		});
     },
     destroy : function( ) {
		return this.each(function(){
     	...
     	if(window.FileReader){
     	    ...
     	    for(i=0; i<this.files.length; i++){
	    		...
	    		reader.onload = function (e) {
	    			$('<div />').css({'background-image': ('url('+e.target.result+')'), 'background-repeat': 'no-repeat', 'background-size': settings['scale'] }).addClass(settings['prefix']+'thumb' + id).appendTo($('#'+settings['prefix']+id));
	    		};
